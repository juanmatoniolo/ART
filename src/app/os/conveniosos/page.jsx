...
export default function ConveniosAdmin() {
  ...
  // Ruta base actualizada para Obras Sociales
  const CONVENIOS_PATH = "facturacionOS/convenios";

  /* ===== Cargar convenios en tiempo real ===== */
  useEffect(() => {
    const conveniosRef = ref(db, CONVENIOS_PATH);
  ...
  const crearDesdePlantilla = async () => {
    ...
    await set(ref(db, `${CONVENIOS_PATH}/${safe}`), newData);
    setNuevoNombre("");
  ...
  const confirmarAumentoPorcentaje = async () => {
    ...
    await set(ref(db, `${CONVENIOS_PATH}/${safeName}`), newData);
  ...
  const guardar = async () => {
    if (!activo) return;
    const refConv = ref(db, `${CONVENIOS_PATH}/${activo}`);
  ...
  const confirmarEliminar = async () => {
    await remove(ref(db, `${CONVENIOS_PATH}/${modalEliminar}`));
  ...
  const confirmarRenombrar = async () => {
    ...
    const snap = await get(ref(db, `${CONVENIOS_PATH}/${modalRenombrar}`));
    if (!snap.exists()) return;
    const data = snap.val();
    const safe = nuevoNombreConvenio.trim().replace(/\s+/g, "_");
    await set(ref(db, `${CONVENIOS_PATH}/${safe}`), data);
    await remove(ref(db, `${CONVENIOS_PATH}/${modalRenombrar}`));
