...
export default function GestionarListasModal({ listas = [], editarLista = null, onGuardar, onEliminar, onClose }) {
    const [form, setForm] = useState(VACIA);
    const [editId, setEditId] = useState(null);
    const [guardando, setGuardando] = useState(false);

    // Inicializar formulario cuando se pasa una lista para editar
    useEffect(() => {
        if (editarLista) {
            setEditId(editarLista.id);
            setForm({
                nombre: editarLista.nombre,
                multiplicador: String(editarLista.multiplicador)
            });
        } else {
            setEditId(null);
            setForm(VACIA);
            ...
            <div className={`${s.modal} ${s.modalWide}`} onClick={e => e.stopPropagation()}>
                <div className={s.modalHeader}>
                    <h3 className={s.modalTitle}>
                        ...
                        {editId ? "Editar lista" : "Nueva lista"}
                    </h3>
                        ...
                        <div className={`${s.gestionBox} ${s.gestionBoxAlt}`}>
                            ...
                            <div className={s.formField}>
                                ...
                                <input
                                    ...
                                    placeholder="Ej: Venta, Mesa de entrada, Facturación"
                                    value={form.nombre}
                            ...
                            <div className={s.formField} style={{ marginTop: ".75rem" }}>
                                ...
                                <div className={s.gestionMultRow}>
                                    ...
                                    <input
                                        ...
                                        onChange={e => set("multiplicador", e.target.value)}
                                    />
                                    <span className={s.gestionHint}>
                                        ({recargo >= 0 ? "+" : ""}{recargo}% sobre el costo)
                                    </span>
                                </div>
                            </div>
                            ...
                            <div style={{ display: "flex", gap: ".5rem", justifyContent: "flex-end" }}>
                                {editId && (
                                    <button className={s.btnCancel} onClick={cancelar}>
                                        Cancelar
                                    </button>
                                )}
                                <button
                                ...
                                >
                                    ...
                                    {guardando ? "Guardando..." : editId ? "Guardar cambios" : "Crear lista"}
                                </button>
                            </div>
                                    ...
                                    .map(l => (
                                        <div key={l.id} className={s.gestionItem}>
                                            <div>
                                                ...
                                                <p className={s.gestionItemMeta}>
                                                    ×{l.multiplicador} · +{Math.round((l.multiplicador - 1) * 100)}%
                                                </p>
                                            </div>
                                            <div className={s.tableActions}>
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        setForm({
                                                            nombre: l.nombre,
                                                            multiplicador: String(l.multiplicador)
                                                        });
                                                    }}
                                                    title="Editar"
                                                >
