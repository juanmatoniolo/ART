...
export async function generarDorsoPDFBlob() {
	const res = await fetch("/DORSO-CX.pdf");
	if (!res.ok) {
		throw new Error(
			"No se encontró /DORSO-CX.pdf. Colocá el archivo en la carpeta /public del proyecto.",
		);
	}
	...
	if (srcPdf.getPageCount() === 0) {
		throw new Error("El PDF DORSO no tiene páginas");
	}

	const newPdf = await PDFDocument.create();
	const [page1] = await newPdf.copyPages(srcPdf, [0]);
