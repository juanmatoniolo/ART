...
export default function ConveniosAdmin() {
  ...
  const editar = (nombre) => {
    ...
    const clavesExistentes = Object.keys(dataNormalizada.valores_generales || {});
    let ordenInicial;
    if (ordenGuardado && Array.isArray(ordenGuardado)) {
      const nuevas = clavesExistentes.filter(k => !ordenGuardado.includes(k));
      ordenInicial = [...ordenGuardado.filter(k => clavesExistentes.includes(k)), ...nuevas];
    } else {
      const ordenadas = ordenValoresGenerales.filter(k => clavesExistentes.includes(k));
      const otras = clavesExistentes.filter(k => !ordenValoresGenerales.includes(k));
  ...
  const guardar = async () => {
    ...
    const payload = normalizeKeys({
      ...editBuffer,
      _orden_valores_generales: editBuffer._orden_valores_generales || Object.keys(editBuffer.valores_generales || {})
    });
  ...
  const descargarExcel = (nombre) => {
    ...
    let todosConceptos;
    if (ordenGuardado && Array.isArray(ordenGuardado)) {
      const nuevas = Object.keys(generales).filter(k => !ordenGuardado.includes(k));
      todosConceptos = [...ordenGuardado.filter(k => generales.hasOwnProperty(k)), ...nuevas];
    } else {
    ...
    const wsDataHonorarios = [
      ...
      ...honorarios.map((row, idx) => [idx + 1, row.Cirujano || 0, row.Ayudante_1 || 0, row.Ayudante_2 || 0])
    ];
    ...
    return `
      <html>
        <head>
          ...
          <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { font-family: 'Segoe UI', Arial, sans-serif; font-size: 10px; color: #1e293b; line-height: 1.3; padding: 10px 15px; background: white; }
  ...
  const imprimirConvenioFormal = () => {
    if (!activo) { alert("Seleccioná un convenio para imprimir"); return; }
    if (!fechaImpresion) { alert("Establecé la fecha de vigencia antes de imprimir."); return; }
    const printWindow = window.open('', '_blank');
    printWindow.document.write(generarHTMLImpresionFormal(editBuffer, activo, fechaImpresion));
  ...
  const imprimirConvenioFormalDesdeLista = (nombre) => {
    ...
    const dataNormalizada = convertirValoresANumero(JSON.parse(JSON.stringify(data)));
    const printWindow = window.open('', '_blank');
    printWindow.document.write(generarHTMLImpresionFormal(dataNormalizada, nombre, fechaImpresion));
    printWindow.document.close();
    printWindow.print();
  };
  ...
  const imprimirConvenioInternoDesdeLista = (nombre) => {
    ...
    const dataNormalizada = convertirValoresANumero(JSON.parse(JSON.stringify(data)));
    const printWindow = window.open('', '_blank');
    printWindow.document.write(generarHTMLImpresionInterno(dataNormalizada, nombre));
    printWindow.document.close();
    printWindow.print();
  };
  ...
  const EditorValores = () => {
    ...
    const handleAddGeneral = () => {
      ...
      setEditBuffer(prev => ({
        ...
        _orden_valores_generales: [...(prev._orden_valores_generales || Object.keys(prev.valores_generales)), safeKey]
      }));
    };
    ...
    const handleDeleteGeneral = (key) => {
      ...
      setEditBuffer(prev => ({
        ...
        _orden_valores_generales: (prev._orden_valores_generales || []).filter(k => k !== key)
      }));
    };

    const todosConceptos = editBuffer._orden_valores_generales || Object.keys(editBuffer.valores_generales || {});
    ...
    const handleDragEnd = (e) => { e.currentTarget.style.opacity = "1"; setDragIndex(null); };
    ...
    const handleDrop = (e, dropIndex) => {
      ...
      setEditBuffer(prev => ({ ...prev, _orden_valores_generales: newOrden }));
      setDragIndex(null);
    };
    ...
    const handleAddHonorario = () => {
      setEditBuffer(prev => ({
        ...prev,
        honorarios_medicos: [...prev.honorarios_medicos, { Cirujano: 0, Ayudante_1: 0, Ayudante_2: 0 }]
      }));
    };
    ...
    const handleDeleteHonorario = (index) => {
      ...
      setEditBuffer(prev => ({
        ...prev,
        honorarios_medicos: prev.honorarios_medicos.filter((_, i) => i !== index)
      }));
    };
      ...
      <div className={styles.editorCard}>
        <div className={styles.editorHeader}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
            <label style={{ color: '#aaa', fontSize: '0.85rem', whiteSpace: 'nowrap' }}>
              Fecha vigencia:
              <input type="date" className={styles.input} value={fechaImpresion}
                onChange={(e) => setFechaImpresion(e.target.value)}
                style={{ marginLeft: '0.5rem', width: 'auto', display: 'inline-block' }} />
            </label>
            <button className={styles.btnSecondary} onClick={imprimirConvenioFormal} title="Imprimir con membrete">🖨️ Formal</button>
            <button className={styles.btnSecondary} onClick={imprimirConvenioInterno} title="Solo tabla de valores">📋 Interno</button>
          </div>
        </div>

        <div className={styles.editorSection}>
          <h5>📋 Valores Generales <span style={{fontSize:'0.75rem', color:'#888'}}>(arrastrá para reordenar)</span></h5>
            ...
            <tbody>
              {todosConceptos.map((key, index) => (
                <tr key={key} draggable
                  ...
                  <td style={{textAlign:'center', color:'#666'}} title="Arrastrar para reordenar">⋮⋮</td>
                  <td>{prettyKey(key)}</td>
                  <td><InputNumero value={toNumber(editBuffer.valores_generales[key])} onChange={(v) => handleGeneralChange(key, v)} /></td>
                  <td><button className={styles.btnDangerSmall} onClick={() => handleDeleteGeneral(key)}>🗑️</button></td>
                </tr>
              ))}
            </tbody>
          </table>
          <button className={styles.btnSecondary} onClick={handleAddGeneral}>➕ Agregar nuevo concepto</button>
        </div>

        <div className={styles.editorSection}>
          <h5>👨‍⚕️ Honorarios Médicos (por complejidad)</h5>
          <p className={styles.hint}>Ayudante 1 = 30% del Cirujano, Ayudante 2 = 20% (solo niveles ≥ 4).</p>
          <table className={styles.editTable}>
            <thead><tr><th>#</th><th>Cirujano</th><th></th></tr></thead>
            <tbody>
              {editBuffer.honorarios_medicos.map((row, index) => (
                <tr key={index}>
                  <td>{index + 1}</td>
                  <td><InputNumero value={toNumber(row.Cirujano)} onChange={(v) => handleCirujanoChange(index, v)} /></td>
                  <td><button className={styles.btnDangerSmall} onClick={() => handleDeleteHonorario(index)}>🗑️</button></td>
                </tr>
              ))}
            </tbody>
          </table>
          <button className={styles.btnSecondary} onClick={handleAddHonorario}>➕ Agregar nivel</button>
        </div>
        ...
        <div className={styles.editorActions}>
          <button className={styles.btnSecondary} onClick={() => setActivo(null)}>Cancelar</button>
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div style={{ background: '#0f172a', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', padding: '0.75rem 1rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
        <label style={{ color: '#aaa', fontSize: '0.9rem' }}>📅 Fecha de vigencia para impresión:</label>
        <input type="date" className={styles.input} value={fechaImpresion} onChange={(e) => setFechaImpresion(e.target.value)} style={{ width: 'auto' }} />
        {fechaImpresion && <span style={{ color: '#6fa17b', fontSize: '0.85rem' }}>({formatearFechaLegible(fechaImpresion)})</span>}
      </div>

      <div className={styles.newRow}>
        <input className={styles.input} placeholder="Nuevo convenio..." value={nuevoNombre} onChange={(e) => setNuevoNombre(e.target.value)} />
        <button className={styles.btnPrimary} onClick={crearDesdePlantilla}>➕ Crear (desde plantilla)</button>
      </div>

      <div className={styles.buttonRow}>
        <button className={styles.btnSecondary} onClick={abrirModalPorcentaje}>📈 Aumentar / Reducir % desde existente</button>
      </div>

      <div className={styles.tableWrapper}>
        <table className={styles.table}>
          <thead><tr><th>Convenio</th><th>Acciones</th></tr></thead>
          <tbody>
            {Object.entries(convenios).map(([nombre, data]) => (
              <tr key={nombre}>
                ...
                <td className={styles.actions}>
                  ...
                  <button onClick={() => imprimirConvenioFormalDesdeLista(nombre)} title="Imprimir Formal">🖨️</button>
                  <button onClick={() => imprimirConvenioInternoDesdeLista(nombre)} title="Imprimir Interno">📋</button>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalAumento}>
            ...
            <div className={styles.modalBody}>
              ...
              <select className={styles.select} value={percentSource} onChange={(e) => setPercentSource(e.target.value)}>
                <option value="">-- Seleccionar --</option>
                {Object.keys(convenios).map(key => <option key={key} value={key}>{prettyKey(key)}</option>)}
              </select>

              <div className={styles.porcentajesGrid}>
                <div><label>% Honorarios</label><input type="number" step="0.1" className={styles.input} value={porcentajeHonorarios} onChange={(e) => setPorcentajeHonorarios(e.target.value)} /></div>
                <div><label>% Gastos</label><input type="number" step="0.1" className={styles.input} value={porcentajeGastos} onChange={(e) => setPorcentajeGastos(e.target.value)} /></div>
              </div>

              <label>Nombre del nuevo convenio</label>
              <input className={styles.input} value={newNamePercent} onChange={(e) => setNewNamePercent(e.target.value)} placeholder="Ej: OCTUBRE_2026_AJUSTADO" />

              <p className={styles.hint}>Los valores se redondearán a enteros. Conceptos de honorarios: Lab, Mod Prep Sangre, FKT, Consulta, Ecografía, ECG.</p>
            </div>
            <div className={styles.modalActions}>
              <button className={styles.btnSecondary} onClick={() => setShowPercentModal(false)}>Cancelar</button>
              <button className={styles.btnPrimary} onClick={confirmarAumentoPorcentaje}>Crear</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalEliminar && (
        <Modal title="Eliminar convenio" message={`¿Eliminar "${prettyKey(modalEliminar)}"?`}
          onCancel={() => setModalEliminar(null)} onConfirm={confirmarEliminar}
          confirmText="Eliminar" confirmClass={styles.btnDanger} />
      )}

      {modalRenombrar && (
        <Modal title="Renombrar convenio" variant="solid"
          message={<><p>Nuevo nombre:</p><input className={styles.input} value={nuevoNombreConvenio} onChange={(e) => setNuevoNombreConvenio(e.target.value)} /></>}
          onCancel={() => setModalRenombrar(null)} onConfirm={confirmarRenombrar}
          confirmText="Renombrar" confirmClass={styles.btnWarning} />
      )}
    </div>
  );
}

function Modal({ title, message, onCancel, onConfirm, confirmText, confirmClass, variant = "default" }) {
  return (
    <div className={`${styles.modalOverlay} ${variant === "solid" ? styles.modalOverlaySolid : ""}`}>
      <div className={`${styles.modal} ${variant === "solid" ? styles.modalSolid : ""}`}>
        <h4>{title}</h4>
